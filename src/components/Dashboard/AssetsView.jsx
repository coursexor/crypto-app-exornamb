...
export default function AssetsView({ holdings, portfolio }) {
  ...
  const totalValueUsd = typeof portfolio?.totalValueUsd === 'number' ? portfolio.totalValueUsd : 0;
  return (
    <div className="max-w-[1000px] mx-auto">
      ...
      <div className="av-summary-row">
        <div className="av-summary-card">
          ...
          <div className="av-summary-value">
            ${safeHoldings.reduce((s, h) => s + (h.avgBuyPrice * h.amountHeld), 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          </div>
        </div>
        <div className="av-summary-card" style={{ borderLeft: `3px solid ${safeHoldings.reduce((s, h) => s + h.profitLossUsd, 0) >= 0 ? '#0EA56A' : '#E53E3E'}` }}>
          <div className="av-summary-label">Total P&L</div>
          <div className="av-summary-value">
            ${safeHoldings.reduce((s, h) => s + h.profitLossUsd, 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          </div>
        </div>
      </div>
      ...
      <div className="av-alloc-bar">
        {safeHoldings.map((h) => {
          const pct = totalValueUsd > 0 ? (h.valueUsd / totalValueUsd) * 100 : 0;
      ...
      <AssetList holdings={safeHoldings} showAll={true} />
    </div>
  );
}
